...
-(void)viewLoad
{
    ...
    if (!([app.conditionID isEqualToString:@"0"])) {
        ...
        [self.myconnection viewIndividualPaperworkProtocol:[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedofficeId"] :app.ruleID :app.conditionID :@"leave"];
    }
...
- (void) keyboardDidShow:(NSNotification *)notification
{
    if (!([[[NSUserDefaults standardUserDefaults]objectForKey:@"keyboardAction"]isEqualToString:@"keyboardEnable"])) {
        ...
        leaveDocumentsClass *ob = (leaveDocumentsClass *)self.superview.superview.superview.superview;
        [ob.scrollView setContentOffset:CGPointMake(ob.scrollView.frame.origin.x,65) animated:YES];
...
{
    NSMutableDictionary *dict=self.finalDict;
    if ([[dict allKeys]containsObject:@"employees"])
    {
        self.countString=[NSString stringWithFormat:@"%lu",(unsigned long)[[dict allKeys]count]-1];
    }
    else
    {
        self.countString=[NSString stringWithFormat:@"%lu",(unsigned long)[[dict allKeys]count]];
    }
    ...
    NSMutableDictionary *protocolDetailsDict=[[NSMutableDictionary alloc]initWithObjectsAndKeys:self.finalDict,@"protocol_details",app.ruleID,@"rule_id",self.descriptionText.text,@"description",@"leave",@"rule_type",[[NSUserDefaults standardUserDefaults] objectForKey:@"selectedofficeId"],@"o_id", nil];
...
-(void)serviceGotResponse:(id)responseData
{
    dispatch_async(dispatch_get_main_queue(), ^{
        if([responseData count] > 0)
        {
            ...
            NSLog(@"%@",responseData);
            self.countString=[responseData objectForKey:@"progress_circle_count"];
            [self.finalDict removeAllObjects];
            [self.grouparray removeAllObjects];
            ...
            for (int i=0; i<allKeysArray.count; i++) {
                ...
                //Checks count of authority for enabling flowImage
        ...
        {
            [self showalerviewcontroller:@"Server Error Occurred"];
        }
