//
//  loanInnerViewClass.m
//  Automate Firm
//
//  Created by leonine on 11/22/16.
...
@implementation loanInnerViewClass
...
-(void)awakeFromNib
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewLoad) name:@"loanPaperworkProtocol" object:Nil];
...
-(void)viewLoad
{
    ...
    if (!([app.conditionID isEqualToString:@"0"])) {
        ...
        [self.myconnection viewIndividualPaperworkProtocol:[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedofficeId"] :app.ruleID :app.conditionID :@"loan"];
    }
...
- (void) keyboardDidShow:(NSNotification *)notification
{
    if(!([[[NSUserDefaults standardUserDefaults]objectForKey:@"keyboardAction"]isEqualToString:@"keyboardEnable"]))
    {
        ...
        loanDocumentViewClass *ob = (loanDocumentViewClass *)self.superview.superview.superview.superview;
...
-(IBAction)saveButtonAction:(id)sender
{
    ...
    NSMutableDictionary *protocolDetailsDict=[[NSMutableDictionary alloc]initWithObjectsAndKeys:self.finalDict,@"protocol_details",app.ruleID,@"rule_id",self.descriptionText.text,@"description",@"loan",@"rule_type",[[NSUserDefaults standardUserDefaults] objectForKey:@"selectedofficeId"],@"o_id", nil];
