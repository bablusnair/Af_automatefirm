//
//  fineInnerViewClass.m
//  Automate Firm
//
//  Created by leonine on 11/23/16.
...
@implementation fineInnerViewClass
...
-(void)awakeFromNib
{
    ...
    p=q=0;
    self.countString=@"0";
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewLoad) name:@"finePaperworkProtocol" object:Nil];
...
-(void)viewLoad
{
    ...
    if (!([app.conditionID isEqualToString:@"0"])) {
        ...
        [self.myconnection viewIndividualPaperworkProtocol:[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedofficeId"] :app.ruleID :app.conditionID :@"fine"];
    }
...
- (void) keyboardDidShow:(NSNotification *)notification
{
    ...
    fineDocumentsViewClass *ob = (fineDocumentsViewClass *)self.superview.superview.superview.superview;
    [ob.scrollView setContentOffset:CGPointMake(ob.scrollView.frame.origin.x,75) animated:YES];
...
-(IBAction)saveButtonAction:(id)sender
{
    ...
    NSMutableDictionary *protocolDetailsDict=[[NSMutableDictionary alloc]initWithObjectsAndKeys:self.finalDict,@"protocol_details",app.ruleID,@"rule_id",self.descriptionText.text,@"description",@"fine",@"rule_type",[[NSUserDefaults standardUserDefaults] objectForKey:@"selectedofficeId"],@"o_id", nil];
...
-(void)createResponse:(id)Response
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if([[[NSUserDefaults standardUserDefaults]objectForKey:@"paperworkAction"]isEqualToString:@"create"])
        {
            fineDocumentTile *ob = (fineDocumentTile *)self.superview.superview;
