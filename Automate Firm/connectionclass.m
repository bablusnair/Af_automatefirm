...
//    [postDataTask resume];
    ...
    NSDictionary *mapData = [[NSDictionary alloc] initWithObjectsAndKeys:@"administrator",@"kp",@"password",@"password",@"E6E5C534-5D2D-4F3F-AE31-8E0B5EH-8C38A",@"deviceid",nil];
    NSData *postData = [NSJSONSerialization dataWithJSONObject:mapData options:0 error:&error];
    [request setHTTPBody:postData];
...
//    [postDataTask resume];
    ...
    NSURLSessionDataTask *postDataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        else if ([httpResponse statusCode]==501)
        {
            if ([self.mydelegate respondsToSelector:@selector(showalerviewcontroller:)]&&(self.mydelegate!=NULL))
            {
                [self.mydelegate showalerviewcontroller:@""];
            }
        }
        else if ([httpResponse statusCode]==500)
        {
