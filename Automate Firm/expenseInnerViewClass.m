...
@implementation expenseInnerViewClass
...
-(void)awakeFromNib
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewLoad) name:@"expensePaperworkProtocol" object:Nil];
...
-(void)viewLoad
{
    ...
    if (!([app.conditionID isEqualToString:@"0"])) {
        ...
        [self.myconnection viewIndividualPaperworkProtocol:[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedofficeId"] :app.ruleID :app.conditionID :@"expense"];
    }
...
- (void) keyboardDidShow:(NSNotification *)notification
{
    if (!([[[NSUserDefaults standardUserDefaults]objectForKey:@"keyboardAction"]isEqualToString:@"keyboardEnable"])) {
        ...
        expenseDocumentViewClass *ob = (expenseDocumentViewClass *)self.superview.superview.superview.superview;
...
-(IBAction)saveButtonAction:(id)sender
{
    ...
    NSMutableDictionary *protocolDetailsDict=[[NSMutableDictionary alloc]initWithObjectsAndKeys:self.finalDict,@"protocol_details",app.ruleID,@"rule_id",self.descriptionText.text,@"description",@"expense",@"rule_type",[[NSUserDefaults standardUserDefaults] objectForKey:@"selectedofficeId"],@"o_id", nil];
...
-(void)createResponse:(id)Response
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([[Response objectForKey:@"status"]isEqualToString:@"200"]) {
            ...
            if([[[NSUserDefaults standardUserDefaults]objectForKey:@"paperworkAction"]isEqualToString:@"create"])
            {
                expenseDocumentTile *ob = (expenseDocumentTile *)self.superview.superview;
